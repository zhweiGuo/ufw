...
class CommonTestCase(unittest.TestCase):
    ...
    def test_set_action(self):
        ...
        for action in ["allow", "deny", "reject", "limit"]:
            r.set_action(action)
            self.assertEquals(action, r.action, "%s != %s" % (action, r.action))
    ...
    def test_set_port(self):
        ...
        for loc in ["dst", "src"]:
            for port in [
                ...
                r.set_port(port, loc)
                if loc == "dst":
                    self.assertEquals(port, r.dport, "%s != %s" % (port, r.dport))
                else:
                    self.assertEquals(port, r.sport, "%s != %s" % (port, r.sport))
    ...
    def test_set_v6(self):
        ...
        for ipv6 in [True, False]:
            r.set_v6(ipv6)
            self.assertEquals(ipv6, r.v6, "%s != %s" % (ipv6, r.v6))
    ...
    def test_set_src(self):
        ...
        for src in ["10.0.0.3"]:
            r.set_src(src)
            self.assertEquals(src, r.src, "%s != %s" % (src, r.src))
    ...
    def test_set_dst(self):
        ...
        for dst in ["10.0.0.3"]:
            r.set_dst(dst)
            self.assertEquals(dst, r.dst, "%s != %s" % (dst, r.dst))
    ...
    def test_set_logtype(self):
        ...
        for logtype in ["", "log", "log-all"]:
            ...
            self.assertEquals(logtype, r.logtype, "%s != %s" % (logtype, r.logtype))
    ...
    def test_normalize(self):
        ...
        for (v6, addr, expected) in data:
            ...
            rule.set_v6(v6)
            rule.normalize()
            self.assertEquals(expected, rule.dst, "'%s' != '%s'" % (expected, rule.dst))
            self.assertEquals(addr != expected, rule.updated, "'%s' not updated" % addr)

            rule = ufw.common.UFWRule("allow", "any", src=addr)
            rule.set_v6(v6)
            rule.normalize()
            self.assertEquals(expected, rule.src, "'%s' != '%s'" % (expected, rule.src))
            self.assertEquals(addr != expected, rule.updated, "'%s' not updated" % addr)
