...
class CommonTestCase(unittest.TestCase):
    def setUp(self):
        self.rules = {
            ...
            "multi-dport": ufw.common.UFWRule("allow", "tcp", dport="80,443,8080:8090"),
            "multi-sport": ufw.common.UFWRule("allow", "tcp", sport="80,443,8080:8090"),
            "multi-both": ufw.common.UFWRule(
    ...
    def test__init_(self):
        ...
        tests.unit.support.check_for_exception(
            self, ufw.common.UFWError, ufw.common.UFWRule, "allow", "nonexistent", "22"
        )
    ...
    def test__get_attrib(self):
        ...
        search = (
            "'-p all -j ACCEPT', action=allow, comment=, dapp=, "
    ...
    def test_format_rule(self):
        ...
        self.assertEqual(
            s,
            "-p all --dport 80 --sport 80 -j DROP "
            + "-m comment --comment 'dapp_Apache,sapp_Apache'",
        )

        s = str(self.rules["dapp"])
        self.assertEqual(
            s, "-p all --dport 80 -j ACCEPT " + "-m comment --comment 'dapp_Apache'"
        )

        s = str(self.rules["full-any"])
        self.assertEqual(
            s, "-p all -d 10.0.0.1 --dport 123 " + "-s 10.0.0.2 --sport 124 -j ACCEPT"
        )

        s = str(self.rules["full-ipv6"])
        self.assertEqual(
            s, "-p ipv6 -d 10.0.0.1 --dport 123 " + "-s 10.0.0.2 --sport 124 -j DROP"
        )

        s = str(self.rules["full-tcp"])
        self.assertEqual(
            s, "-p tcp -d 10.0.0.1 --dport 123 " + "-s 10.0.0.2 --sport 124 -j LIMIT"
        )

        s = str(self.rules["full-udp"])
        self.assertEqual(
            s, "-p udp -d 10.0.0.1 --dport 123 " + "-s 10.0.0.2 --sport 124 -j REJECT"
        )
        ...
        self.assertEqual(
            s, "-p tcp -m multiport " + "--dports 80,443,8080:8090 -j ACCEPT"
        )

        s = str(self.rules["multi-sport"])
        self.assertEqual(
            s, "-p tcp -m multiport " + "--sports 80,443,8080:8090 -j ACCEPT"
        )
        ...
        self.assertEqual(
            s, "-p all --sport 80 -j DROP " + "-m comment --comment 'sapp_Apache'"
        )
    ...
    def test_set_action(self):
        ...
        for action in ["allow", "deny", "reject", "limit"]:
            r.set_action(action)
            self.assertEqual(action, r.action, "%s != %s" % (action, r.action))
    ...
    def test_set_port(self):
        ...
        for loc in ["dst", "src"]:
            for port in [
                ...
                r.set_port(port, loc)
                if loc == "dst":
                    self.assertEqual(port, r.dport, "%s != %s" % (port, r.dport))
                else:
                    self.assertEqual(port, r.sport, "%s != %s" % (port, r.sport))
    ...
    def test_set_port_bad(self):
        ...
        for loc in ["dst", "src"]:
            for port in [
                ...
                if port == 22:
                    e = TypeError
                tests.unit.support.check_for_exception(self, e, r.set_port, port, loc)
    ...
    def test_set_protocol(self):
        ...
        for proto in ["any", "tcp", "udp", "ipv6", "esp", "ah", "vrrp"]:
            ...
            self.assertEqual(proto, r.protocol, "%s != %s" % (proto, r.protocol))
    ...
    def test_set_protocol_bad(self):
        ...
        for proto in ["an", "cp", "up", "nonexistent"]:
            tests.unit.support.check_for_exception(
                self, ufw.common.UFWError, r.set_protocol, proto
            )
    ...
    def test_set_v6(self):
        ...
        for ipv6 in [True, False]:
            r.set_v6(ipv6)
            self.assertEqual(ipv6, r.v6, "%s != %s" % (ipv6, r.v6))
    ...
    def test_set_src(self):
        ...
        for src in ["10.0.0.3"]:
            r.set_src(src)
            self.assertEqual(src, r.src, "%s != %s" % (src, r.src))
    ...
    def test_set_src_bad(self):
        ...
        for src in ["10.0.0.", "10..0.0.3"]:
            tests.unit.support.check_for_exception(
                self, ufw.common.UFWError, r.set_src, src
            )
    ...
    def test_set_dst(self):
        ...
        for dst in ["10.0.0.3"]:
            r.set_dst(dst)
            self.assertEqual(dst, r.dst, "%s != %s" % (dst, r.dst))
    ...
    def test_set_dst_bad(self):
        ...
        for dst in ["10.0.0.", "10..0.0.3"]:
            tests.unit.support.check_for_exception(
                self, ufw.common.UFWError, r.set_dst, dst
            )
    ...
    def test_set_interface_bad(self):
        ...
        for if_type in ["ina", "ot"]:
            tests.unit.support.check_for_exception(
                self, ufw.common.UFWError, r.set_interface, if_type, interface
            )
    ...
    def test_set_position_bad(self):
        ...
        r = self.rules["any"]
        tests.unit.support.check_for_exception(
            self, ufw.common.UFWError, r.set_position, "a"
        )
    ...
    def test_set_logtype(self):
        ...
        for logtype in ["", "log", "log-all"]:
            ...
            self.assertEqual(logtype, r.logtype, "%s != %s" % (logtype, r.logtype))
    ...
    def test_set_logtype_bad(self):
        ...
        for logtype in ["a", "loga", "d"]:
            tests.unit.support.check_for_exception(
                self, ufw.common.UFWError, r.set_logtype, logtype
            )
    ...
    def test_set_direction(self):
        ...
        for direction in ["in", "out"]:
            ...
            self.assertEqual(
                direction, r.direction, "%s != %s" % (direction, r.direction)
            )
    ...
    def test_set_direction_bad(self):
        ...
        for direction in ["", "ina", "outta"]:
            tests.unit.support.check_for_exception(
                self, ufw.common.UFWError, r.set_direction, direction
            )
    ...
    def test_normalize(self):
        ...
        for rule in keys:
            ...
            r.normalize()
            self.assertEqual(
                ufw.common.UFWRule.match(self.rules[rule], r),
                0,
        ...
        bad.src = "1000.0.0.1"
        tests.unit.support.check_for_exception(self, ufw.common.UFWError, bad.normalize)
        bad = None
        bad = ufw.common.UFWRule("allow", "any")
        bad.dst = "1000.0.0.1"
        tests.unit.support.check_for_exception(self, ufw.common.UFWError, bad.normalize)
        ...
        for (v6, addr, expected) in data:
            ...
            rule.set_v6(v6)
            rule.normalize()
            self.assertEqual(expected, rule.dst, "'%s' != '%s'" % (expected, rule.dst))
            self.assertEqual(addr != expected, rule.updated, "'%s' not updated" % addr)

            rule = ufw.common.UFWRule("allow", "any", src=addr)
            rule.set_v6(v6)
            rule.normalize()
            self.assertEqual(expected, rule.src, "'%s' != '%s'" % (expected, rule.src))
            self.assertEqual(addr != expected, rule.updated, "'%s' not updated" % addr)
...
def test_main():  # used by runner.py
    tests.unit.support.run_unittest(CommonTestCase)
