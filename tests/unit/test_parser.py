...
class ParserTestCase(unittest.TestCase):
    ...
    def test_ufwcommand_parse_empty(self):
        ...
        tests.unit.support.check_for_exception(self, ValueError, c.parse, [])

    def test_ufwcommand_help(self):
        """Test UFWCommand.help()"""
        c = ufw.parser.UFWCommand("basic", "status")
        tests.unit.support.check_for_exception(self, ufw.common.UFWError, c.help, [])
    ...
    def test_ufwparser_register_command(self):
        ...
        tests.unit.support.check_for_exception(
            self, ufw.common.UFWError, parser.register_command, c
        )
    ...
    def test_ufwparser_allowed_command(self):
        ...
        # Invalid commands
        tests.unit.support.check_for_exception(
            self, ValueError, self.parser.allowed_command, "basic", "nonexistent"
        )

        tests.unit.support.check_for_exception(
            self, ValueError, self.parser.allowed_command, "nonexistent", "allow"
        )
    ...
    def test_ufwcommand_rule_get_command(self):
        ...
        for cmd in cmds:
            ...
            if cmd[0] == "rule":
                ...
                res = ufw.parser.UFWCommandRouteRule.get_command(pr.data["rule"])
            ...
            if (
                "from" in cmd_compare
                and cmd_compare[cmd_compare.index("from") + 1] == "any"
                and (
                    len(cmd_compare) - 2 == cmd_compare.index("from")
                    or (
                        cmd_compare.index("from") + 2 < len(cmd_compare)
                        and cmd_compare[cmd_compare.index("from") + 2] != "port"
                        and cmd_compare[cmd_compare.index("from") + 2] != "app"
                    )
                )
            ):
            ...
            # above)
            if (
                ("from" in cmd_compare or "proto" in cmd_compare or "on" in cmd_compare)
                and "to" in cmd_compare
                and cmd_compare[cmd_compare.index("to") + 1] == "any"
                and (
                    len(cmd_compare) - 2 == cmd_compare.index("to")
                    or (
                        cmd_compare.index("to") + 2 < len(cmd_compare)
                        and cmd_compare[cmd_compare.index("to") + 2] != "port"
                        and cmd_compare[cmd_compare.index("to") + 2] != "app"
                    )
                )
            ):
            ...
            if (
                "to" in cmd_compare
                and "from" not in cmd_compare
                and cmd_compare[cmd_compare.index("to") + 1] == "any"
                and cmd_compare.index("to") + 2 < len(cmd_compare)
                and "on" not in cmd_compare
            ):
                if "port" in cmd_compare:
                    port = "%s" % cmd_compare[cmd_compare.index("port") + 1]
                    if "proto" in cmd_compare:
                        port += "/%s" % cmd_compare[cmd_compare.index("proto") + 1]
            ...
            if (
                "to" not in cmd_compare
                and "from" not in cmd_compare
                and "on" not in cmd_compare
                and (
                    "proto" in cmd_compare
                    or cmd_compare[-1].startswith("log")
                    or cmd_compare[-1] in generics
                )
            ):
                if "proto" in cmd_compare:
            ...
            if (
                cmd_compare[0] == "route"
                and "out" in cmd_compare
                and "in" in cmd_compare
                and cmd_compare.index("out") < cmd_compare.index("in")
            ):
            ...
            if "%s %s" % (cmd[0], res) != compare_str:
                errors.append(
                    ' "%s %s" != "%s" (orig=%s)' % (cmd[0], res, compare_str, cmd)
                )
    ...
    def test_misc_rules_parse(self):
        ...
        cmds = [
            ["delete", "allow", "22"],
            ["deny", "from", "any", "port", "domain", "to", "any", "port", "tftp"],
            ["allow", "to", "any", "proto", "gre"],
            ["deny", "to", "any", "proto", "ipv6"],
            ["allow", "to", "any", "proto", "igmp"],
            ["reject", "to", "any", "proto", "esp"],
            ["deny", "to", "224.0.0.1", "proto", "igmp"],
            ["deny", "in", "on", "eth0", "to", "224.0.0.1", "proto", "igmp"],
            ["allow", "in", "on", "eth0", "to", "192.168.0.1", "proto", "gre"],
    ...
    def test_rule_bad_syntax(self):
        ...
        cmds = [
            ...
            (["rule"], ValueError),
            (["rule", "allow", "22", "in", "on", "eth0"], ufw.common.UFWError),
            (["rule", "allow", "in", "in", "eth0", "22"], ufw.common.UFWError),
            ...
            (["rule", "reject", "in", "on", "eth0", "port", "22"], ufw.common.UFWError),
            (["rule", "allow", "to", "any", "22"], ufw.common.UFWError),
            (["rule", "allow", "to", "any", "to", "22"], ufw.common.UFWError),
            ...
            (
                ["rule", "allow", "from", "any", "port", "nonexistent", "proto", "any"],
                ufw.common.UFWError,
            ),
            (
                [
            ...
            (
                ["rule", "deny", "to", "any", "port", "tftp", "proto", "tcp"],
                ufw.common.UFWError,
            ),
            (["rule", "deny", "to", "::1", "proto", "ipv6"], ufw.common.UFWError),
        ...
        for cmd, exception in cmds:
            ...
            # the cmd, not a reference
            tests.unit.support.check_for_exception(
                self, exception, self.parser.parse_command, cmd + []
            )
    ...
    def test_simple_bad_numeric_port(self):
        ...
        for port in ["-1", "1000000"]:
            c = ["rule", "allow", port]
            tests.unit.support.check_for_exception(
                self, ufw.common.UFWError, self.parser.parse_command, c
            )
    ...
    def test_bad_simple_action(self):
        ...
        for action in ["allw", "eny", "nonexistent"]:
            c = ["rule", action, "22"]
            tests.unit.support.check_for_exception(
                self, ValueError, self.parser.parse_command, c
            )
    ...
    def test_bad_simple_action_with_direction(self):
        ...
        c = ["rule", "allow", 5, "22"]
        tests.unit.support.check_for_exception(
            self, AttributeError, self.parser.parse_command, c
        )
...
def test_main():  # used by runner.py
    tests.unit.support.run_unittest(ParserTestCase)
