...
def initvars(install_dir):
    ...
    ufw.common.config_dir = os.path.join(os.path.realpath(topdir), "ufw/etc")
    ufw.common.state_dir = os.path.join(os.path.realpath(topdir), "ufw/lib/ufw")
    ufw.common.share_dir = os.path.join(os.path.realpath(topdir), "ufw/usr/share/ufw")
...
def check_for_exception(t, expectedException, func, *args):
    try:
        ...
        t.fail(
            "Unexpected exception thrown for '%s%s:\n%s"
            % (str(func), str(args), sys.exc_info()[0])
        )
    else:
...
def get_sample_rule_commands_extended(v6=False):
    ...
    if v6:
        dsts = [
            "",
            "2001:db8:85a3:8d3:1319:8a2e:370:7341",
            "1234:db8::/32",
            "any",
        ...
        srcs = [
            "",
            "2001:db8:85a3:8d3:1319:8a2e:370:7342",
            "5678:fff::/64",
            "any",
                                ...
                                for comment in ["", "thumbs 👍"]:
                                    ...
                                    if proto:
                                        try:
                                            ...
                                            if (
                                                dport not in apps
                                                and sport not in apps
                                                and (
                                                    (dport == "fsp" and proto == "udp")
                                                    or (
                                                        sport == "fsp"
                                                        and proto == "udp"
                                                    )
