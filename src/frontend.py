...
def parse_command(argv):
    ...
    # Rule commands
    rule_commands = ["allow", "limit", "deny", "reject", "insert", "delete", "prepend"]
    ...
    if len(argv) > 2:
        ...
        if argv[idx].lower() == "--dry-run":
            idx = 2
        if (
            argv[idx].lower() != "default"
            and argv[idx].lower() != "route"
            and argv[idx].lower() in rule_commands
        ):
...
class UFWFrontend:
    ...
    def set_enabled(self, enabled):
        ...
        if changed:
            try:
                self.backend.set_default(
                    self.backend.files["conf"], "ENABLED", config_str
                )
        ...
        if enabled:
            ...
            if error_str != "":  # pragma: no cover
                ...
                try:
                    self.backend.set_default(
                        self.backend.files["conf"], "ENABLED", "no"
                    )
                except UFWError as e:
                    error(e.value)
                        ...
                        if len(matching) > 0:
                            ...
                            for i in matching:
                                if i > 0 and i - 1 < len(rules):
                                    res += "   [%2d] %s\n" % (
                                        i,  # Don't need UFWCommandRule here either
    ...
    def get_show_added(self):
        ...
        for r in self.backend.get_rules():
            if r.forward:
                rstr = "route %s" % ufw.parser.UFWCommandRouteRule.get_command(r)
            else:
                ...
                if self.backend.use_ipv6():
                    if ip_version == "v4":
                        ...
                        if user_pos == -1:  # prepend
                            begin = 0 if count == 0 and num_v6 == 0 else 1
                            r.set_position(begin)
                        elif not r.remove and r.position > 0 and r.position <= num_v4:
                            # The user specified a v4 rule, so try to find a
                            # match in the v6 rules and use its position.
                            p = self.backend.find_other_position(r.position, False)
                            if p > 0:
                        ...
                        if not r.remove and r.position > num_v4 and user_pos != -1:
                            r.set_position(r.position - num_v4)
    ...
    def delete_rule(self, number, force=False):
        ...
        if not force:
            ...
            if ans != "y" and ans != self.yes.lower() and ans != self.yes_full.lower():
                proceed = False
    ...
    def application_add(self, profile):
        ...
        if default == "skip":
            ufw.util.debug(
                "Policy is '%s', not adding profile '%s'" % (policy, profile)
            )
            return rstr
        elif default == "accept":
            policy = "allow"
        ...
        if "rule" in pr.data:
            rstr = self.do_action(pr.action, pr.data["rule"], pr.data["iptype"])
        else:
